								...
								<div class="widget widget-table action-table">
									...
									<div class="widget-content">
										...
										<table class="table table-striped table-bordered" id="search_table">
											...
											<tbody>
												<?php
													$sel_inv = "select * from invoice";
													$res_inv = sqlsrv_query($conn, $sel_inv);
													while($row_inv = sqlsrv_fetch_array($res_inv)){
														$inv_id = $row_inv['ID'];
														$invoice_id = 'DLI/INV/'. (1001 + $row_inv['ID']);
														$lr_ids = $row_inv['lr_ids'];
														$sel_lr = "select lr.*, CONVERT(VARCHAR(10), lr.LR_DATE,105) as LR_DATE from lr where lr.ID in (".$lr_ids.") order by lr.LR_DATE DESC";
														//echo $sel_lr;
