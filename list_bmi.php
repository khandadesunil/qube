<?php
	...
	//sqlsrv_query($conn, 'TRUNCATE TABLE lr');
	//sqlsrv_query($conn, 'TRUNCATE TABLE process');
	//sqlsrv_query($conn, 'TRUNCATE TABLE invoice');
	//sqlsrv_query($conn, 'ALTER TABLE lr ADD DELIVERYCHARGE decimal(9,2)');
	//sqlsrv_query($conn, "update lr set INVOICE = '', INVOICE_ID = ''");
	//die;
?>
<div class="main">
								...
								<div class="widget widget-table action-table">
									...
									<div class="widget-content">
										<table class="table table-striped table-bordered" id="search_table">
											...
											<tbody>
												<?php
													$selbmo = "select p.*, CONVERT(VARCHAR(10), p.BMI_DATE,105) as BMI_DATE from process p where p.STATUS = 'BMI' order by p.BMO_DATE DESC";
													//echo $selbmo;
													...
													while($rowbmo = sqlsrv_fetch_array($resbmo)){
														...
														$BMI_DATE = $rowbmo['BMI_DATE'];
														
														$sel_lr = "select LR_NUMBER, CONVERT(VARCHAR(10), LR_DATE,105) as LR_DATE, CONSR, (select NAME from city where ID = FROM_BRANCH) as FROM_BRANCH, (select NAME from city where ID = TO_BRANCH) as TO_BRANCH 
