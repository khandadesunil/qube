								...
								<div class="widget widget-table action-table">
									<div class="widget-header">
										...
										<?php
											if($sess_user_type != 'READONLY')
												echo '<span class="right_link"><a href="add_lr.php">Add Booking</a></span>';
										?>
									...
									<div class="widget-content">
										<table class="table table-striped table-bordered" id="search_table">
											...
											<tbody>
												<?php
													$sel_lr = "select lr.*, CONVERT(VARCHAR(10), lr.LR_DATE,105) as LR_DATE from lr where 1=1 order by lr.LR_DATE DESC";
													//echo $sel_lr;
												...
												<tr>
													...
													<td><?=$t_br?></td>
													<td><?=$NO_OF_PKGS?></td>
													<td><a href="print_lr.php?id=<?=$ID?>" target="_blank" class="btn btn-primary">Print</a></td>
													<?php
														if($sess_user_type != 'READONLY'){
													?>
													<td><a href="edit_lr.php?id=<?=$ID?>" class="btn btn-small btn-success"><i class="icon-large icon-pencil"> </i></a></td>
													<?php
													}
														if($_SESSION['type'] == 'ADMIN'){
													?>
