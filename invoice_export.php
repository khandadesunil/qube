<?php
	...
	/*$sql = 	"select lr.INVOICE, invoice.invoice_date, lr.LR_NUMBER, lr.LR_DATE, lr.CONSR, lr.shipper, (select name from city where ID = lr.FROM_BRANCH) as FROM_BRANCH, 
			...
			lr.NO_OF_PKGS, lr.QUANTITY, lr.TOTAL from lr, invoice where lr.INVOICE_ID = invoice.id and lr.INVOICE != '' ".$condi;*/
	$sql = 	"select lr.INVOICE, invoice.invoice_date, lr.LR_NUMBER, CONVERT(VARCHAR(10), lr.LR_DATE,110) as LR_DATE, lr.CONSR, lr.shipper, (select name from city where ID = lr.FROM_BRANCH) as FROM_BRANCH,
			(select name from city where ID = lr.TO_BRANCH) as TO_BRANCH, GOODS, lr.NO_OF_PKGS, lr.QUANTITY, lr.TOTAL
			from lr 
			left outer join invoice on invoice.id = lr.INVOICE_ID
			where lr.INVOICE != ''".$condi;
	//echo $sql;die;
