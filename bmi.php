<?php
	...
	if($action != ''){
		...
		$bmiids = rtrim ($bmiids , ",");
		if($bmiids != ''){
			$sql = "insert into process(BMI_NUMBER, BMI_DATE, LR_ID, STATUS, CREATED_BY) 
						values('".$BMI_NUMBER."', '".$BMI_DATE."', '".$bmiids."', 'BMI', '".$CREATED_BY."')";
			//echo $sql;
			$res = sqlsrv_query($conn, $sql);
			if ($res) {
				$msg = '<div style="color:green;">BMI successful!!</div>';
				...
				$em = rtrim($em, ', ');
				$to = $em;
				$subject = 'Qube Services - Your shipment is Reached';
				
				$message = 'Dear Sir, <br /><br />';
				$message .= 'Your shipment is Reached';
				...
				$msg = '<div style="color:red;">Sorry!! Could not add BMI!!</div>';
			}
		}else{
			$msg = '<div style="color:red;">There is no booking selected for BMI</div>';
		}
	}
?>
<div class="main">
							...
							<div class="widget big-stats-container">
								<div class="widget widget-table action-table">
									...
									<div class="widget-content">
										...
										<table class="table table-striped table-bordered" id="search_table">
											...
											<tbody>
												<?php
													...
													$lrids = rtrim($lrids, ',');
													$sel_lr = "select * from lr where STATUS = 'BMO' and ID in (".$lrids.")";
													//echo $sel_lr;
												...
												?>
												<tr>
													<td><input type="checkbox" name="bmi_ids[]" value="<?=$ID?>"></td>
													<td><?=$LR_NUMBER?></td>
													<td><?=$BMO_NUMBER?></td>
													<td><?=$LR_DATE?></td>
													<td><?=$SHIPPER?></td>
													...
													<td><?=$BMO_DATE?></td>
													<!--<td><?=$STATUS?></td>-->
													<td><input type="text" name="email[]" value="<?=$CONSR_EMAIL?>" class="span3"></td>
												</tr>
												<?php
													$i = $i + 1;
													}
										...
										<input type="submit" name="add" id="add" value="BMI" class="btn btn-primary"><br /><br />
										</form>
									</div>	
								</div>	
							</div>	
