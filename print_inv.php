<?php
	...
	if($inv_id != ''){
		$sel_inv = "select *, CONVERT(VARCHAR(10), invoice_date,105) as invoice_date from invoice where id = ".$inv_id;
		$res_inv = sqlsrv_query($conn, $sel_inv);
		$row_inv = sqlsrv_fetch_array($res_inv);
		//print_r($row_inv);
		$lr_ids = $row_inv['lr_ids'];
		$sel_lr = "select lr.*, CONVERT(VARCHAR(10), lr.LR_DATE,105) as LR_DATE from lr where lr.ID in (".$lr_ids.")";
		...
		$sel_lr = "select lr.*, CONVERT(VARCHAR(10), lr.LR_DATE,105) as LR_DATE from lr where lr.ID in (".$ID.")";
		$res_lr = sqlsrv_query($conn, $sel_lr);
		$row_lr = sqlsrv_fetch_array($res_lr);
		$CONSR = $row_lr['CONSR'];
...
<style>table {border-collapse: collapse;}table, tr, th { border: 0px solid black;height:20px;}tr{font-size:12px;font-family:"Arial";font-weight:600;}table{border:0.1px solid;}td{padding:5px;}</style>
	<table width="700" border="1" cellspacing="0" cellpadding="0">
		...
		<tr style="height:110px;">
			...
			<td colspan="3" align="right" style="border:0px solid !important;">Invoice No: <?=$inv_id = 'DLI/INV/' . (1000 + $inv_id)?><br />Date : <?=$invoice_date?></td>
		</tr>
		...
		<tr style="height:40px;">
			<td><?=$i?></td>
			<td><?=$GOODS.' ('.$LR_NUMBER.')<br />From : '.$F_NAME.' To : '.$T_NAME.'<br /> Shipper : '.$SHIPPER?></td>
